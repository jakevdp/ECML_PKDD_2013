...
\textcolor{red}{Point out that any external estimator can be used as base
estimator thanks to duck typing}\\
...
\subsection{Extending \sklearn}

Any object that conforms to a small superset of the API
lined out in Section~\ref{sec:core-api},
(in particular, that implements methods \texttt{get\_params}
and \texttt{set\_params} for setting hyperparameters;
refer to the documentation at \url{http://scikit-learn.org} for details)
can be used as a \sklearn estimator
and be ``plugged into'' a pipeline, grid search or other meta-estimator
To define such objects, a user can either derive a new class
from \texttt{sklearn.base.BaseEstimator},
or define the entire class from scratch,
without even an \texttt{import sklearn}
in the relevant program module.
The Python principle of ``duck typing''
is employed to make sure that even instances of a class
that does not derive from any \sklearn class can be a first-class estimator.

In other places of the library,
in particular in the feature extraction and vectorization code
for text documents,
the toolkit is designed to be extensible.
Here, the estimators provide ``hooks'' for user-defined code:
objects or functions that provide a specific API
can be given as arguments at vectorizer construction time.
\sklearn code then calls into this code
to obtain the desired behavior.
Again, such user code can be kept agnostic
of the \sklearn class hierarchy,
operating on standard Python objects such as strings instead.

The design principle here is that user code
should not be tied to \sklearn,
which is a \textit{library}, not a \textit{framework}.
This principle avoids a variant of the well-known problem
with object-oriented design,
which is that users wanting a ``banana'' get
``a gorilla holding the banana and the entire jungle''
(Joe~Armstrong, interviewed by \citealp{seibel2009coders}:213):
a program that is not reusable with another machine learning toolkit
(or other solution to the problem at hand),
because it is intimately tied to \sklearn.

\section{Implementation}
\label{sec:implementation}

% \subsection{Cython for performance}

Our implementation guidelines emphasize on writing short and readable code,
easily maintainable by external contributors, while being as efficient as
possible.

Whenever practicable, algorithms implemented in \sklearn are thus
directly written in pure Python. For intensive numerical computations though,
the library mostly relies on NumPy vector operations, which allows for the code
to remain concise, readable and efficient. For all algorithms that cannot be
easily and efficiently expressed as NumPy operations, we rely on Cython
\citep{behnel2011cython}  for writing the critical parts and achieving decent
performance and scalability. Cython is a compiled programming language that
extends Python with static typing. It produces efficient C extension modules
that are directly importable from the Python run-time system. Examples of
algorithms written in Cython include stochastic gradient descent for linear
models, some graph-based clustering algorithms or decision trees.  A problem
arising from the use of Cython, however, is that passing pure Python functions
as argument, while possible, degrades performance significantly due to the
run-time checks needed for each function invocation. This is especially
problematic for example for kernel methods, which would benefit from accepting a
kernel function as argument. A potential solution to this problem would be for
\sklearn to provide a Cython-level API, thereby allowing the user to pass
Cython-implemented kernel functions.

To facilitate the adoption of the library, we also try to limit to a bare
minimum the set of external dependencies. As such, NumPy and SciPy -- the two
most common dependencies in the Python scientific ecosystem -- are actually the
only two required dependencies for the library to fully work. Some modules may
rely on other external libraries, but in such cases we tend to prefer to
directly ship them within the codebase rather than adding new dependencies for
the user to install. In particular, the codebase includes modified versions of
the \textsf{LIBSVM} and \textsf{LIBLINEAR} libraries
\citep{chang2011libsvm,fan2008}, both written in C{}\verb!++! and wrapped using
Cython modules.

Finally, some estimators in \sklearn also support multi-core architectures.
Machine learning algorithms can indeed often be decomposed into independent
smaller tasks that may each be solved in parallel, thereby reducing the overall
computing times. At this point in time, support for multi-core programming in
Python -- at least in the CPython implementation targeted by scikit-learn -- is
however suboptimal. The interpreter supports multithreading, but due to the
\textit{global interpreter lock}, no two threads in a process may execute Python
code at the same time. It follows that multithreading is not a viable solution
for parallelizing parts of an algorithm, even for embarrassingly parallel
problems, such as fitting and evaluating multiple models in a grid search
procedure. The workaround that we use is to rely on the Python multiprocessing
module, which emulates threads using multiple operating system- level processes.
However this solution is not perfect as it may in turn lead to a large memory
overhead since objects (e.g., input datasets) need to be copied within each
process own memory space. In some cases, it may unfortunately even happen that
the overhead becomes so significant that fully exploiting a multi-core
architecture becomes infeasible due memory constraints. As such, a proper
implementation of parallel machine learning algorithms remain a challenging task
for the project.
...
which, as described in Section~\ref{sec:implementation}, is only implemented at a
...
\textcolor{red}{Mention BSD license, INRIA and stuff.}
