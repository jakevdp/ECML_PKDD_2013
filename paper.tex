...
The \sklearn project \citep{pedregosa2011} provides an open source machine learning
...
general-purpose high-level language. It includes classical learning algorithms,
model evaluation and selection tools, as well as preprocessing procedures.
...
and functions that users import into Python programs.
No command-line interface, let alone a graphical user interface,
is offered for non-programmer users. Instead,
interactive use is made possible by the Python interactive interpreter, and its
enhanced replacement IPython \citep{perez2007ipython}, which offers a
\textsc{matlab}-like working environment especially suited for scientific use.
...
Since 2007, \sklearn has been developed by an international team of more than a dozen
...
In \sklearn, the data representation is kept as close as possible
to the matrix representation: datasets are encoded as
two-dimensional NumPy arrays or SciPy sparse matrices.
While these may seem rather unsophisticated data representations when
...
These formats also provide a collection of
data loading and conversion tools which make them very easy to use in many
contexts. For tasks where the inputs are text files or semi-structured
...
estimator) decreases the ease of use and is also more likely to confuse newcomers.
...
into existing estimator classes
