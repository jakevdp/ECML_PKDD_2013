...
They take as input an existing estimator
...
The code in Figure~\ref{svm-gridsearch} illustrates how to find
...
communicating with it by passing objects of standard Python/NumPy types,
...
\textit{library}, and not a framework. This principle avoids a
common problem with object-oriented design, which is that users wanting a
...
Visualization functionality uses on Matplotlib \citep{hunter2007matplotlib}
...
(and sometimes do not offer any kind of API).
While these packages have the advantage
that their users are not tied to a particular programming language,
users will find that they still need programming to process input/output,
