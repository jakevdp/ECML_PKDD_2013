...
Furthermore, every estimator allows \texttt{fit(X\_train).transform(X\_train)}
...
this may either skip only an input validation step,
...
\end{pythoncode}
The \texttt{OutputCodeClassifier} object
\textit{clones} the LR estimator multiple times for learning,
resulting in a set of estimator objects with the same settings
that each perform a binary classification
(see \citealt{dietterich1995solving} for the algorithm).
The snippet thus exemplifies the importance
...
As an example, the snippet below illustrates how to find the
hyper-parameter settings of an SVM classifier (SVC)
that maximize its $F_1$ score,
