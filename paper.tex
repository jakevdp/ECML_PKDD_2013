...
\textit{scikit-image}\footnote{\url{http://scikit-image.org}}, an
...
a widely used representation in machine learning: each row in the
...
Many machine learning packages exist, with new ones still often appearing,
that are intended to be used as command-line tools
(and sometimes do not offer any type of API).
Examples include Weka \citep{hall2009weka}, SofiaML \citep{sculley2010web}
and Vowpal~Wabbit \citep{langford2009sparse}.
While these packages have the advantage
that their users are not tied to a particular programming language,
the users will find that they still need to do programming,
and will do so in a variety of languages.
We argue that this makes it harder for researchers and developers
to exchange software and collaborate on software.
...
To maximize ease of use, we encourage researchers
to follow these conventions when releasing machine learning software.
