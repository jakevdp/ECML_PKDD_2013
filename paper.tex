...
directly written in pure Python. For intensive numerical computations, the
library mostly relies on NumPy vector operations, which allows for the code to
remain concise and readable while being efficient. For all algorithms that
...
the adoption of the library, we also limit to a bare minimum the set of external
dependencies. As such, NumPy and SciPy -- the two most common dependencies in the
Python scientific ecosystem -- are actually the only two required dependencies
for the library to fully work. Some modules may rely on other external
libraries, but then we tend to prefer to directly ship them within the codebase
rather than adding new dependencies. In particular, the codebase ships modified
versions of the \textsf{LIBSVM} and \textsf{LIBLINEAR} libraries
