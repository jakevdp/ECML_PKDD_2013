...
benchmarked against several competitors. In this paper, we instead present an
in-depth analysis of design choices made when building the
...
\begin{description}
  ...
  \item[Non-proliferation of classes.]
       ...
       Datasets are represented as standard Python list, NumPy array or
       Scipy sparse matrices.
       Hyper-parameter names and values are represented as standard
       Python string or number whenever possible.
...
some output variable $Y$. A sample is then defined as a pair of input/output
values $([x_1,..., x_p]^\mathrm{T}, y)$. of these variables. A widely used
representation of a dataset, a collection of
such samples, is a set of two matrix of numerical values: one for the input
values and one for the output values. Each row of these matrix
...
tree with too small values of the maximal tree depth
parameter  will lead to underfitting, while too large
values will make it overfit.
