...
These default values are set to be relevant in many common
...
From the start, the choice to let a single object serve dual purpose as
estimator and model has mostly been driven by usability and technical
considerations. From the user point of view, having two coupled instances (i.e.,
an estimator object, used as a factory, and a model object, produced by the
estimator) indeed decreases the ease of use and is also more likely to
unnecessarily confuse newcomers. From the developer point of view, decoupling
estimators from models also creates parallel class hierarchies and increases the
overall maintenance complexity of the project. For these practical reasons, we
believe that decoupling estimators from models is not worth the effort. A good
reason for decoupling however, would be that it makes it possible to ship a
model in a new environment without having to deal with potentially complex
software dependencies. Such a feature could however still be implemented in
\sklearn by making estimators able to export a fitted model, using the
information from its public attributes, to an agnostic model description such as
PMML~\citep{guazzelli2009pmml}.
...
Finally, predictors must provide a \texttt{score} function to assess their
performance on a batch of input data. In supervised estimators, this method
...
% XXX do we need a subsection header here?
% "Recursive composition of estimators"?
Figure~\ref{nested-gridsearch} shows how different meta-estimators interact.
This snippet builds a composite estimator in several steps.
First, a linear SVM classifier is combined in a pipeline with
a feature selector based on $\chi^2$ test statistics.
This is then fed as input to a \texttt{OneVsRestClassifier}
to build a $K$-way multiclass estimator
consisting of $K$ binary classifiers,
after determining the value of $K$ at \texttt{fit} time
from the training labels.
Each of these binary classifiers performs feature selection
independently of all the others.
This composite classifier is then combined
with a \textsf{tf--idf} feature extractor using yet another pipeline.
Because the result is again an estimator,
we can perform grid search to find optimal values for the hyper-parameters
of each step of the recursive pipeline.

The \texttt{param\_grid} in Figure~\ref{nested-gridsearch}
uses a \sklearn-specific notation
to address the individual components of the nested structure.
This notation uses double underscores to separate components,
so that \texttt{tfidf\_\_sublinear\_tf}
addresses the \texttt{sublinear\_tf} setting
of the \textsf{tf--idf} feature extractor
(when turned on, term frequencies are replaced by their logarithms).
Note the \texttt{estimator} component,
which address the base estimator (the first pipeline)
inside the OvR meta-estimator.
This is required because the wrapper may itself have settings
that might conflict with the estimator that it wraps;
this is not required for pipelines,
because those have no settings of their own.
...
param_grid = {
  ...
  "ovr__estimator__chi2__k": [200, 500, 1000],
  "ovr__estimator__svm_C": [1, 10, 100]}

clf = GridSearchCV(doc_clf, param_grid, cv=10)
\end{pythoncode}
\caption{Parameter search with nested pipelines for text classification.}
\label{nested-gridsearch}
\end{figure}
...
Recent years have witnessed a rising interest in machine learning and data
mining with applications in many fields. With this rise comes a host of machine
learning packages (both open source and proprietary) with which \sklearn
competes. Some of those, including Weka~\citep{hall2009weka} or
