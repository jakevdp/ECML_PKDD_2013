...
If a one-vs.-one scheme is desired instead,
this can achieved using a \texttt{OneVsOneClassifier} meta-estimator
wrapping a \texttt{LogisticRegression} estimator:
\begin{pythoncode}
ovo_lr = OneVsOneClassifier(LogisticRegression(penalty="l1"))
\end{pythoncode}
The \texttt{OneVsOneClassifier} object
...
that each perform a binary classification and vote to make the final decision.
