...
example in supervised learning task, the goal is to find a mapping
between input variables $X_1, ..., X_p$, called features, and
some discrete as in classification task or continous as in regression task
output variables $Y$. A sample is then defined as a pair of
...
The choice to let a single object serves dual purpose as estimator and model
...
coefficient of determination and mean squared error for regression.
...
when \texttt{kernel} is set to a radial basis function kernel \texttt{"rbf"}.
An SVM with a linear kernel accepts a \texttt{gamma} parameter, but ignores it,
