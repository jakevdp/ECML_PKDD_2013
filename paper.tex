...
simplifies the \sklearn API
compared to an approach where estimators and models are separated,
forming two parallel class hierarchies.
While from a mathematical point of view,
models are more similar to one another than algorithms,
we argue that this is less true in practical machine learning.
As an example, consider the case of linear models,
described mathematically by a weight matrix $w$ and an intercept vector $b$,
suggesting a class \texttt{LinearModel}
with such a matrix and vector as attributes.
However, to use such a model in practice,
one first needs to know whether the task at hand is
regression, $y = w^\top x + b$,
or classification, $y = \arg \max_k[w_{:,k}^\top x + b_k]$,
calling for two classes.
Then, the various linear probabilistic classifiers must be distuingished:
logistic regression, with $P(y \mid x) = \sigma(w^\top x + b)$,
multinomial naïve Bayes, with $\log P(y \mid x) = w^\top x + b$,
and the winnow probability estimator \citep{zhang2002}, with
$P(y \mid x) = \frac{1}{2} (1 + \mathrm{median} \{-1, w^\top x + b, 1 \})$,
would all need either distinct classes,
or a common class that remembers the training regime.
Furthermore, separating estimators and models
would make online/out-of-core learning
(not further discussed in the present paper) more complicated.
