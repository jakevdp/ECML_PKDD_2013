...
Since meta-estimators require the construction of nested objects,
...
An SVM with a linear kernel could accept a $\gamma$ parameter,
but would ignore it and thus waste computing time trying out effectively
equivalent settings if a single hyper-parameter grid was used insteead.
...
construction time. The library then calls into this code to obtain the desired behavior.
