...
Initializing and fitting an estimator object
proceeds in a way that is similar to partial function application:
an estimator can be considered a function
that maps hyperparameter settings to an actual learning algorithm.
The advantage of this API
over one where the object initializer would perform the learning
will become especially apparent in Section~\ref{sec:advanced-api}.
...
The return value of \texttt{fit} is always the object it was called on,
which now serves as a model of its input
and can be used to perform predictions or transformations of input data.
...
Additionally, by leveraging that fact that \texttt{fit}
always returns the estimator it was called on,
the \texttt{StandardScaler} example above
can be rewritten in a single line using a ``method chaining'' style:
\begin{pythoncode}
X_train = StandardScaler().fit(X_train).transform(X_train)
\end{pythoncode}
Finally, \texttt{fit(X\_train).transform(X\_train)} can further
