...
Initializing of estimator objects and actual learning are strictly separated,
in a way that is similar to partial function application:
an estimator is initialized from a set of hyper-parameter settings
(e.g., regularization parameters, random seeds)
and can be considered a function
that maps these settings to actual learning algorithms.
The object initializer does not see any actual data.
All it does is attach the given parameters to the object.
Hyper-parameter settings are public attributes
for the sake of convenient model inspection,
which is especially important in model selection settings.
For ease of use, default hyper-parameter values are provided
...
the return value always being the object it was called on,
which can be used to perform predictions or transformations of input data.
