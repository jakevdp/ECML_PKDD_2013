...
To ease code reuse, object-oriented techniques and design patterns are also used
all throughout the codebase. In particular, the \textit{duck typing} design
pattern is widely exploited to simplify implementation and skip the introduction
of superfluous classes. This technique indeed allows both for extensibility and
flexibility: user-defined estimators that follow our API and conventions should
be reusable (e.g., in a pipeline) as any built-in estimators, without them
actually inheriting from our base classes.
...
that are directly importable from the Python runtime system. Examples of
...
runtime checks needed for each function invocation. This is especially
...
\sklearn to provide a Cython-level API, thereby alllowing the user to pass
...
directly ship them within the codebase rather than adding new dependencies. In
particular, the codebase includes modified versions of the \textsf{LIBSVM} and
\textsf{LIBLINEAR} libraries \citep{chang2011libsvm,fan2008}, both written in
C{}\verb!++! and wrapped using Cython modules.

Finally, some algorithms in \sklearn also support multi-core architectures.
